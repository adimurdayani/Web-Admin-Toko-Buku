...
class TransaksiController extends Controller
{
    ...
    {
        $gettransaksis = Transaksi::with(['costumer'])->whereHas('costumer', function ($query) use ($id) {
            $query->whereId($id);
        })->orderBy("id", "desc")->get();
    ...
    {
        $transaksi  = Transaksi::with(['details.produk', 'costumer'])->where('id', $id)->first();
        if ($transaksi) {
            $transaksi->update([
                'status' => "BATAL"
            ]);
    ...
    public function pusNotif($title, $message, $mfcm)
    {
        $mData = [
            'title' => $title,
            'body' => $message
